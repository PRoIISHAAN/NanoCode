// The vocabulary and shape here (a lean orchestration core, per decisions/0003-orchestration-core.md)
// is informed by studying prime-agent's equivalent contracts, but written independently against
// nanocode's own dependency: `@earendil-works/pi-ai@0.84.4`'s actual published API surface, which
// differs from what an implementation written against an unpublished/older snapshot would assume.
// Two consequences of that:
//
// 1. `StreamFn` binds to `Models.streamSimple`'s signature -- the method on the `MutableModels`
//    collection @nanocode/ai builds via `createModelsRegistry()`, which resolves provider auth
//    internally -- rather than a standalone `streamSimple` function. pi-ai's own compat.d.ts says
//    outright: "Temporary compatibility entrypoint preserving the old global pi-ai API surface ...
//    This module is deleted with the coding-agent ModelManager migration. ... new code uses
//    createModels() and the provider factories." That also means no `getApiKey`/`apiKey` config
//    surface is needed here: the bound `Models` instance already knows how to authenticate
//    whichever provider a call targets.
// 2. `ServiceTier` isn't exported from pi-ai's top-level index (it lives under
//    `api/openai-responses-shared.ts`, an OpenAI-Responses-specific concept) -- dropped from
//    `AgentState` for now rather than reached for via a subpath import nanocode doesn't need yet.
import type {
  Api,
  AssistantMessage,
...
/** Stream function used by the agent loop -- bind `Models.streamSimple` from @nanocode/ai to this. */
 ...
 * thinking-level metadata from @nanocode/ai to detect support for a concrete model.
 */
