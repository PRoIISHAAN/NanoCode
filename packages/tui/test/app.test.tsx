...
describe("App", () => {
  it("renders text deltas as they stream in, then the settled final message", async () => {
    ...
    const { lastFrame } = render(
      <App session={session} setup={NEVER_CALLED_SETUP} version="0.0.0-test" cwd="/test/cwd" />,
    );
  ...
  it("does not resurrect stale streaming text after message_end settles it (backpressure race)", async () => {
    ...
    const { lastFrame } = render(
      <App session={session} setup={NEVER_CALLED_SETUP} version="1.0.0" cwd="/test" />,
    );
  ...
  it("shows a startup banner and a status bar with real cwd/model/reasoning, updating on real usage", async () => {
    ...
    const { lastFrame, stdin } = render(
      <App session={session} setup={NEVER_CALLED_SETUP} version="1.2.3" cwd="/home/me/project" />,
    );
    ...
    expect(frame).toContain("↑0 ↓0 tokens");
    expect(frame).toContain("0.0% of 200,000 ctx"); // FAKE_MODEL.contextWindow
    ...
    expect(frame).toContain("↑2,000 ↓50 tokens");
    expect(frame).toContain("1.0% of 200,000 ctx");
  ...
  it("starts in onboarding with no session, then switches to a genuinely working RunningSession once setup.finish() resolves", async () => {
    ...
    const { lastFrame, stdin } = render(
      <App session={undefined} setup={controller} version="0.0.0-test" cwd="/test/cwd" />,
    );
    await wait(10);
